...
(function() {
  ...
  // Load COGA when on dashboard route (Electron: COGA is already loaded via renderer.js)
  function loadCOGA() {
    // In Electron, COGA is loaded via renderer.js, so just check if it's available
    if (getCOGAInstance()) {
      cogaLoaded = true;
      setupDashboard();
      return;
    ...
    // If COGA is not available yet, wait for it (it's loaded by renderer.js)
    if (cogaLoading) {
      return;
    }
    
    cogaLoading = true;
    console.log('[COGA] Dashboard: Waiting for COGA to load...');
    
    // Wait for COGA to become available (max 10 seconds)
    let attempts = 0;
    ...
    const checkCOGA = function() {
      ...
      if (coga) {
        cogaLoaded = true;
        cogaLoading = false;
        console.log('[COGA] Dashboard: COGA found, setting up dashboard');
        setupDashboard();
      } else if (attempts < maxAttempts) {
        setTimeout(checkCOGA, 100);
      } else {
        cogaLoading = false;
        console.error('[COGA] Dashboard: COGA not available after waiting');
      }
    };
